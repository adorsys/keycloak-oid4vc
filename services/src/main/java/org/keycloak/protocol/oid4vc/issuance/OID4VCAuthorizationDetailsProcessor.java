...
public class OID4VCAuthorizationDetailsProcessor implements AuthorizationDetailsProcessor<OID4VCAuthorizationDetail> {
    ...
    private OID4VCAuthorizationDetail buildAuthorizationDetail(ClientSessionContext clientSessionCtx, OID4VCAuthorizationDetail requestAuthDetail) {
        ...
        CredentialScopeModel credScope = findCredentialScopeModelByConfigurationId(
                realmModel, clientSessionCtx::getClientScopesStream, requestedCredentialConfigurationId);
        if (credScope == null) {
            throw getInvalidRequestException(
                    "Cannot find or access client scope for credential_configuration_id: " + requestedCredentialConfigurationId);
        }

        OID4VCAuthorizationDetail responseAuthDetail =
                CredentialScopeModelUtils.buildOID4VCAuthorizationDetail(credScope, null);
        responseAuthDetail.setClaims(requestAuthDetail.getClaims());
