...
public class JwtProofValidator extends AbstractProofValidator {
    ...
    void validateNoPrivateKeyInHeaderClaims(String algorithm, Map<String, Object> headerClaims) {
        ...
        SignatureProviderFactory signatureProviderFactory = (SignatureProviderFactory) keycloakSession.getKeycloakSessionFactory()
                .getProviderFactory(SignatureProvider.class, algorithm);

        Set<String> privateClaimNames = signatureProviderFactory != null
                ? signatureProviderFactory.getJwkPrivateKeyClaims()
                : Collections.emptySet();

        if (privateClaimNames.isEmpty()) {
            // Fallback to the current set of claims if no provider is found, or it doesn't define any
            // This ensures we keep the same level of security as before
            privateClaimNames = Set.of("d", "p", "q", "dp", "dq", "qi", "oth", "k");
        }
