...
| `86400`
| When strategy is `randomize`, pick a timestamp within the last N seconds from the current time, never shifting into the future relative to the original value.
...
- For SD-JWT VC, time claims (`iat`, `nbf`, `exp`) are typically set by protocol mappers. Use the mapper described below to write normalized values.
